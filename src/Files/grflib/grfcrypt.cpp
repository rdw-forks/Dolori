...
static uint8_t *DES_Permutation(uint8_t *block, const uint8_t *table) {
  ...
  for (i = 0; i < 0x40; i++) {
    tmp = table[i] - 1;
    if (block[tmp >> 3] & BitMap[tmp & 7]) tmpblock[i >> 3] |= BitMap[i & 7];
  }
...
static uint8_t *DES_RawProcessBlock(uint8_t *block, const uint8_t *ks) {
  ...
  for (i = 0; i < 0x30; i++) {
    tmp = DES_E[i] + 0x1F;
    if (block[tmp >> 3] & BitMap[tmp & 7]) tmpblock[0][i / 6] |= BitMap[i % 6];
  }

  /* bitwise XOR the keyschedule against the expanded block */
  for (i = 0; i < 8; i++) tmpblock[0][i] ^= ks[i];
...
static uint8_t *DES_ProcessBlock(uint8_t rounds, uint8_t *dst,
  ...
  if (rounds > 0) {
    for (i = 0; i < rounds; i++) {
      DES_RawProcessBlock(
          dst, (uint8_t *)ks + (dir == GRFCRYPT_DECRYPT ? 0xF - i : i) * 8);

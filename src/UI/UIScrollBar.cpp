...
void CUIScrollBar::OnLBtnDown(int x, int y) {
  if (m_maxPos) {
    ...
    if (m_isVert) {
      switch (hit) {
        case 0:
          m_parent->SendMsg(this, 7, -1, 0, 0, 0);
          break;
        case 1:
      ...
      switch (hit) {
        case 2:
          ...
          break;
        case 0:
          m_parent->SendMsg(this, 8, -1, 0, 0, 0);
          break;
        case 1:
  ...
  if (scrollbar_length != 3 * m_scrollBtnSize) {
    ...
    if (g_WindowMgr->GetCapture() == this && m_parent) {
      if (!m_isVert) {
        ...
        m_deltaDrag += m_parent->SendMsg(this, 8, button_drag, 0, 0, 0);
        return;
      } else {
        ...
        if ((m_maxPos + 1) * (y - m_startDragY) / pos_scroll_end !=
          ...
          button_drag = (m_maxPos + 1) * (y - m_startDragY) / pos_scroll_end -
                        m_deltaDrag;
          m_deltaDrag += m_parent->SendMsg(this, 7, button_drag, 0, 0, 0);
          return;
        }
      }
    }
